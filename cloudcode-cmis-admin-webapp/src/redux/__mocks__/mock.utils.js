  ...
  searchInArray(_incomingArray, _queryObj, _filtrationFields = []) {
    ...
    _filtrationFields.forEach(item => {
      if (item in _queryObj) {
        ...
        _incomingArray = resultBuffer.slice(0); //use of slice[0] to create a completely new array obj containing the first element of resultBuffer array in mem without mutating the original val
        resultBuffer = [].slice(0); //use of slice[0] to create a completely new empty array obj in mem without mutating the original val
        firstIndexes = [].slice(0); //use of slice[0] to create a completely new empty array obj in mem without mutating the original val
      }
    });
